...
std::string DeckCodec::encode(const DeckDesign& deck_design)
{
   ...
   // Encode
   for(auto& gofX : reverse(groups_of_Xs)) {
      // build the lists of set and faction combinations within the groups of similar card
      auto groups = _group_cards(gofX);
      // to ensure that the same decklist in any order produces the same code, sort them
      _sort_groups(groups);
      _encode_group(result, groups);
   }
...
std::vector< std::vector< CardToken > > DeckCodec::_group_cards(std::vector< CardToken >& cards)
{
   ...
   while(not cards.empty()) {
      ...
      // the weird lambda capturing is due to structured bindings [set, region,...] not being
      // explicitly capturable by lambdas in C++17. The assignment is a workaround
      auto ptr_to_start_erase = std::remove_if(
         ...
         [&set = set, &region = region, &current_set = current_set](const auto& card) {
            ...
            if(this_set == set && this_region == region) {
               // asside from removing the cards from the list, also store them in the current set
               // vector
   ...
   std::string& bytes, const std::vector< std::vector< CardToken > >& groups)
{
   ...
   for(const auto& group : groups) {
      ...
      // now the cards within this group, as identified by the third section of card code only now,
   ...
   std::vector< std::vector< CardToken > >& group_of)
{
   ...
   auto alphanumeric_cmp = [](const CardToken& dc1, const CardToken& dc2) {
...
void DeckCodec::_encode_Nof(std::string& bytes, const std::vector< CardToken >& ofNs)
{
   for(const auto& deck_card : ofNs) {
